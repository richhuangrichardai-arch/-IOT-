 ...
 * Running on [http://192.168.137.186:5000](http://192.168.137.186:5000)

[請在此處插入終端機執行成功的截圖]

📱 使用者操作
拿出手機或電腦，連接至與樹莓派相同的 Wi-Fi。
開啟瀏覽器，輸入網址：http://192.168.137.186:5000
進入 「智慧冷氣中控」 介面進行操作。
...
GPIO 初始化：
設定模式為 BCM。
關鍵安全設定：啟動時將所有 GPIO 輸出設為 LOW (0V)，確保繼電器保持斷開狀態，防止誤觸發。
Web 路由 (Routes)：
GET / : 渲染操作介面。
...
AJAX 非同步通訊：使用 JavaScript fetch API 發送指令，操作時頁面不需刷新，體驗流暢。
